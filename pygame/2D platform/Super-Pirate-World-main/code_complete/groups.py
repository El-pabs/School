...
# Define the WorldSprites class, which is a group of sprites in the game world
class WorldSprites(pygame.sprite.Group):
    ...
    def draw(self, target_pos):
        ...
        # Draw background and main sprites
        for sprite in sorted(self, key = lambda sprite: sprite.z):
            if sprite.z < Z_LAYERS['main']:
                self.display_surface.blit(sprite.image, sprite.rect.topleft + self.offset)
            elif sprite.z == Z_LAYERS['main']:
                self.display_surface.blit(sprite.image, sprite.rect.topleft + self.offset)

# Define the AllSprites class, which is a group of all sprites in the game
class AllSprites(pygame.sprite.Group):
    def __init__(self, width, height, clouds, horizon_line, bg_tile = None, top_limit = 0):
        ...
        # Create background tiles or sky
        if bg_tile:
    ...
    # Constrain the camera to the game world
