...
# boule
DIM_BOULE = 10
boule = pygame.Rect(width/2, height/2, DIM_BOULE, DIM_BOULE)
vitesse_boule = 5
...
hauteur_separateur = height/N
offset = 20

# créer les pointillés
for i in range(N):
    x = width/2 - largeur_separateur/2
...
#   rebondir sur les top et bottom
    if boule.top <= 0 or boule.bottom >= height:
...
def initalisation_boule(boule, boule_dir):
    boule.x = width/2
    boule.y = randint(int(height / 4), int(height - height / 4))
...
#   dessiner les scores
    screen.blit(joueur_droite.afficher_score(), (width/2 + 20, 10))
    screen.blit(joueur_gauche.afficher_score(), (width/2 - 40, 10))
...
joueur_gauche = Joueur((5, height/2 - Joueur.HAUTEUR/2))
joueur_droite = Joueur((width - Joueur.LARGEUR - 5, (height/2 - Joueur.HAUTEUR/2)))
...
def main():
    ...
    while True:
        ...
        if game_running:
            ...
            if keys[pygame.K_s] and joueur_gauche.rect.y < height - Joueur.HAUTEUR:
                joueur_gauche.deplacer_bas()
            if keys[pygame.K_UP] and joueur_droite.rect.y > 0:
                joueur_droite.deplacer_haut()
            if keys[pygame.K_DOWN] and joueur_droite.rect.y < height - Joueur.HAUTEUR:
            ...
            pong_pos = ((width - pong_text.get_width()) // 2, (height - pong_text.get_height()) // 2 - 125)
            player_win_pos = ((width - player_win_text.get_width()) // 2, (height - player_win_text.get_height()) // 2 - 75) if player_win_text else None
            play_message_pos = ((width - play_message_text.get_width()) // 2, (height - play_message_text.get_height()) // 2 + 25)
            time_played_pos = ((width - time_played_text.get_width()) // 2, play_message_pos[1] - 50) if time_played_text else None
